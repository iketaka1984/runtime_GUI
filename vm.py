...
def execution(mode,lock,mlock,command,opr,start,end,stack,address,value,tablecount,rstack,lstack,rtop,ltop,endflag,parpath,fbtmode,count_pc,variable_region,p1_pc,p2_pc,value_conflict):
    pc=start
    pre=pc
    top=len(stack)
            ...
            if fbtmode!='q':
                ...
                if mode == '2':
                    ...
                    with open("variables.txt",'w') as f:
                        f.write(""+str(value[0:tablecount.value])+"")
    ...
    if fbtmode=='f':
        ...
        if parflag!=0:
            if fbtmode=='q':
                ...
                mode= mode_select
            #exec initial part
            stack=execution(mode,lockfree,lockfree,com,opr,0,start[0],stack,address,value,tablecount,rstack,lstack,rtop,ltop,endflag0,0,fbtmode,count_pc,variable_region,p1_pc,p2_pc,value_conflict)
            if mode=='2':
                for i in range(0,parflag,1):
                    lock[i].acquire()
                process={}
                ...
                for i in range(0,parflag,1):        
                    process[i]=Process(target=execution,args=(mode,lock[i],mlock,com,opr,start[i],end[i],stack,address,value,tablecount,rstack,lstack,rtop,ltop,endflag[i],i+1,fbtmode,count_pc,variable_region,p1_pc,p2_pc,value_conflict))
            if mode=='1':
                process={}
                #generate parallel process
                for i in range(0,parflag,1):        
                    process[i]=Process(target=execution,args=(mode,lock[0],mlock,com,opr,start[i],end[i],stack,address,value,tablecount,rstack,lstack,rtop,ltop,endflag[i],i+1,fbtmode,count_pc,variable_region,p1_pc,p2_pc,value_conflict))
            for i in range(0,parflag,1):
                    process[i].start()
            #monitor
            if mode=='2':
            ...
            with open("stdcash.txt",'w') as f:
                f.write("")
            execution(mode,lockfree,lockfree,com,opr,end[parflag-1]+1,len(com),stack,address,value,tablecount,rstack,lstack,rtop,ltop,endflag0,0,fbtmode,count_pc,variable_region,p1_pc,p2_pc,value_conflict)
            if mode_select == '1':
            ...
            mode='1'
            execution(mode,lockfree,mlock,com,opr,0,len(com),stack,address,value,tablecount,rstack,lstack,rtop,ltop,endflag0,0,fbtmode,count_pc,variable_region,p1_pc,p2_pc,value_conflict)
        ...
        if parflag!=0:
            ...
            for i in range(0,parflag,1):
                lock[i].acquire()
            process={}
            execution(mode,lockfree,lockfree,com,opr,0,end[0],stack,address,value,tablecount,rstack,lstack,rtop,ltop,endflag0,0,fbtmode,count_pc,variable_region,p1_pc,p2_pc,value_conflict)
            #generate process
            with open("stdcash.txt","w") as f:
                f.write("")
            for i in range(0,parflag,1):
                process[i]=Process(target=execution,args=(mode,lock[parflag-i-1],mlock,com,opr,end[i],start[i],stack,address,value,tablecount,rstack,lstack,rtop,ltop,endflag[i],parflag-i,fbtmode,count_pc,variable_region,p1_pc,p2_pc,value_conflict))
            for i in range(0,parflag,1):
                process[i].start()
            a='2'
            if len(ldata)==0:
            ...
            with open("stdcash.txt",'w') as f:
                f.write("")
            execution(mode,lockfree,lockfree,com,opr,start[parflag-1]+1,count_pc,stack,address,value,tablecount,rstack,lstack,rtop,ltop,endflag0,0,fbtmode,count_pc,variable_region,p1_pc,p2_pc,value_conflict)
            ...
            execution(mode,lockfree,lockfree,com,opr,0,len(com),stack,address,value,tablecount,rstack,lstack,rtop,ltop,endflag0,0,fbtmode,count_pc,variable_region,p1_pc,p2_pc,value_conflict)
    elif fbtmode=='t':
        (start,end,com,opr,count_pc,parflag,variable_region)=coderead(start,end,com,opr,count_pc,parflag,fbtmode,variable_region)
        forward(ltop.value,rtop.value,fbtmode,value,lstack,rstack,count_pc,com,opr)
